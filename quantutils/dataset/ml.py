...
def evaluate(predictions, data_y, threshold=0.5, display=True):
    ...
    if display:
        ...
        print("Information Coefficient : " + str(((num / den) * 2) - 1))
...
def aggregatePredictions(predictions_list, method="vote_unanimous_all"):
    ...
    if (
        method == "vote_majority"
    ):  # Vote rather than average (removes effects of outliers) across all data. Voting will ignore the strength of the individual prediction.
        a = np.sign(a - 0.5)
        for predictions in predictions_list[1:]:
            a = a.add(np.sign(predictions - 0.5))
        result = (
            (a.sum(axis=1) / (predictions_list[0].shape[1] * len(predictions_list) * 2))
            + 0.5
        ).to_frame()
    if (
        method == "vote_unanimous_pred"
    ):  # Vote and Remove conflicting predictions across sub-predictions
        a = np.sign(np.sign(a - 0.5).sum(axis=1))
        for predictions in predictions_list[1:]:
            a = a.add(np.sign(np.sign(predictions - 0.5).sum(axis=1)))
        result = (a / (len(predictions_list) * 2) + 0.5).to_frame()
    if (
        method == "vote_unanimous_markets"
    ):  # Vote and Remove conflicting predictions across markets
        a = np.sign(a - 0.5)
        for predictions in predictions_list[1:]:
            a = a.add(np.sign(predictions - 0.5))
        a = (
        ...
        result = a[a != 0.5].to_frame()
    if (
        method == "vote_unanimous_all"
    ):  # Vote and Remove conflicting predictions across sub-predictions and markets
        a = np.sign(np.sign(a - 0.5).sum(axis=1))
        for predictions in predictions_list[1:]:
            a = a.add(np.sign(np.sign(predictions - 0.5).sum(axis=1)))
        a = (a / ((len(predictions_list) * 2))) + 0.5
        result = a[a != 0.5].to_frame()
    return result
...
def toTradeSignals(predictions, threshold=0):
    ...
    signals[
        (a == 1)
    ] = -1  # Set any DOWN signals to -1 (UP signals will pick up the default of 1)
